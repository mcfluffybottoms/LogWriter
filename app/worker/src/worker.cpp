...
worker::worker(const std::string &filename, const std::string &default_loglevel,
               size_t number_of_threads)
    : logger(std::make_unique<logWriter::logger>(
          filename, level::str_to_loglevel(default_loglevel))),
      running_(true) {
...
void worker::worker_loop() {
  ...
  while (running_ || !q.empty()) {
    std::unique_ptr<virtual_task> t;
    {
      ...
      cv_.wait(lock, [this]() { return !running_ || !q.empty(); });
      if (!running_ && q.empty()) {
        return;
      }
      if (q.empty())
