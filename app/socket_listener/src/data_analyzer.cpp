...
void dataCollector::get_data(char *buffer) {
  ...
  snprintf(buffer, 256,
           ...
           "Max length: %zu\n"
           "Average length: %zu",
           num_of_msg.load(), num_DEBUG.load(), num_INFO.load(),
           num_CRIT.load(), logs_lasthour.size(), min_len.load(),
           max_len.load(), avrg_len.load());
}
void dataCollector::cleanup(std::chrono::system_clock::time_point now) {
  while (!logs_lasthour.empty() &&
         logs_lasthour.front() < now - std::chrono::hours(1)) {
    logs_lasthour.pop();
  }
}

receiver::receiver(int port, size_t N, size_t T)
    : interval_messages(N), interval(T) {
...
void receiver::start() {
  ...
  while (running_) {
    ...
    if (collector.num_of_msg % interval_messages == 0 && running_) {
      ...
      std::cout << "-------- REPORT AFTER " << interval_messages
                << " MESSAGES --------\n";
