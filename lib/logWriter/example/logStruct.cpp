...
struct testStruct {
  testStruct(int i, const std::string &s)
      : value_int(i), value_string(s), streamObj(i) {}

  int throwError() { throw std::runtime_error("Got RunmtimeError!"); }
  int returnInteger() { return value_int; }
  const std::string &returnString() { return value_string; }
  const simpleStream &returnSimpleStream() { return streamObj; }
...
int main() {
  ...
  try {
    logManager.log("Initializing testStruct with data 1 and \"string\". ",
