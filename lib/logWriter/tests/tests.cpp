...
void test_log() {
  ...
  runner.runTest("Several Inputs", [&runner, &expected]() {
  ...
  runner.runTest("Filters Out Logging Levels", [&runner, &expected]() {
  ...
  runner.runTest("Simple Change Default Logging Levels", [&runner,
                                                          &expected]() {
    ...
    logger.log("LOGGER MESSAGE 3", level::logLevel::CRITICAL);
    // CHANGE TO CRITICAL
    logger.set_default_loglevel(level::logLevel::CRITICAL);
    runner.assertEqual(level::logLevel::CRITICAL,
                       logger.get_default_loglevel());
    logger.log("LOGGER MESSAGE 1", level::logLevel::DEBUG);
    logger.log("LOGGER MESSAGE 2", level::logLevel::INFO);
    logger.log("LOGGER MESSAGE 3", level::logLevel::CRITICAL);
    // CHANGE TO DEBUG
    logger.set_default_loglevel(level::logLevel::DEBUG);
    runner.assertEqual(level::logLevel::DEBUG, logger.get_default_loglevel());
    logger.log("LOGGER MESSAGE 1", level::logLevel::DEBUG);
    logger.log("LOGGER MESSAGE 2", level::logLevel::INFO);
    logger.log("LOGGER MESSAGE 3", level::logLevel::CRITICAL);
    // CHECK RESULT
